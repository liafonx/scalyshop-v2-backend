...
router.get("/api/products", function (req, res, next) {
    Product.find()
        .then((products) => {
            const enableSorted = unleash.isEnabled("enable-sorted");
            console.log("enableSorted>>>", enableSorted);
            let _products = products; // Initialize _products with products

            if (enableSorted) {
                _products = products?.sort((a, b) => b.price - a.price);
                productSortCounter.inc({ version: 'price' }); // Increment counter for price sort
            } else {
                productSortCounter.inc({ version: 'original' }); // Increment counter for original sort
            }

            res.json({ products: _products });
        })
        .catch((err) => {
            return next(err);
        });
